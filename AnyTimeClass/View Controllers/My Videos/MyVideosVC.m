...
@interface MyVideosVC ()<CAAnimationDelegate>{
    NSMutableArray  *VideoIDArray,*videoArray;
    NSArray *results;
    BOOL isSearch;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CoursesInfo *info = [isSearch ? results : videoArray objectAtIndex:indexPath.row];
    [cell.titleImageView sd_setImageWithURL:[NSURL URLWithString:info.videothumbnail] placeholderImage:[UIImage imageNamed:@"img1"]];
    cell.subjectLabel.text = info.videoName;
    cell.typeLabel.text = info.subjectName;
    [VideoIDArray addObject:info.videoID];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    MyVideosDetailsVC *myVC = [[MyVideosDetailsVC alloc] initWithNibName:@"MyVideosDetailsVC" bundle:nil];
    myVC.videoID = [VideoIDArray objectAtIndex:indexPath.row];
...
- (void)onTextChange :(NSString *)searchText{
    ...
    results = [videoArray filteredArrayUsingPredicate:predicate];
    if (results.count) {
...
-(void) callApiForVideosList {
    ...
    [[ServiceHelper_AF3 instance] makeWebApiCallWithParameters:user_dict AndPath:kmyVideos WithCompletion:^(BOOL suceeded, NSString *error, id response) {
        ...
        if (suceeded) {
            ...
            if (responseCode.integerValue == KSuccessCode) {
                ...
                for (NSDictionary  *subjectDict in subjectListArray) {
                    [videoArray addObject :[CoursesInfo parseVideoList:subjectDict]];
                }
